...
export default function Navbar() {
  const { t, language, hasHydrated } = useTranslation();
  const settings = useSettingsStore();

  const lang = hasHydrated ? language : 'EN';
  const currency = hasHydrated ? settings.currency : 'USD';
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50">
      ...
      <div className="flex items-center space-x-6 text-sm font-semibold text-gray-700">
        ...
        <Link 
          href="/my-trips"
          className="flex items-center space-x-2 hover:bg-red-50 hover:text-red-600 px-3 py-2 rounded-lg transition-colors group"
        >
