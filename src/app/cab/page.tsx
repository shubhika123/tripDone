...
export default function CabPage() {
  ...
  useEffect(() => {
    if (!searchState.from || !searchState.to || !searchState.selectedModes.includes('cab')) {
      router.replace('/')
    }
  }, [searchState, router])
    ...
    <div className="min-h-screen bg-gray-50/50 pb-28 font-sans">
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 flex items-center">
              ...
              Departure Booking: Cab
            </h1>
            <p className="text-sm font-medium text-gray-500 mt-1">
               Choose your ride to complete your journey.
            </p>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm mb-8 w-full md:w-max">
           <label className="block text-sm font-bold text-gray-900 mb-2">Select Pickup Time <span className="text-red-500">*</span></label>
        ...
        <div className={`space-y-4 ${isBookedExternally ? 'opacity-50 pointer-events-none grayscale transition-all' : ''}`}>
           {sortedCabs.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
                ...
                <p className="text-xl text-gray-500 font-medium">No cabs available for this route.</p>
                <p className="text-sm text-gray-400 mt-2">Try skipping this step if booked externally.</p>
              </div>
           ) : (
             <>
                 <h2 className="text-2xl font-black text-gray-900 mb-6">Available Cabs</h2>
           ...
           <div>
             {isBookedExternally ? (
               <div className="font-bold text-gray-900 flex items-center gap-2">
                 <CheckCircle className="text-green-500 w-5 h-5"/> Skipping Cab Step
               </div>
             ) : searchState.selectedCab ? (
               <div>
                  <span className="text-gray-500 font-semibold text-sm">Selected Ride:</span>
                  <div className="font-black text-gray-900 text-lg flex items-center gap-2">
                     {searchState.selectedCab.type || 'Sedan'} • ₹{(searchState.selectedCab.price || 0).toLocaleString()}
                  </div>
               </div>
             ) : (
               <div className="text-gray-500 font-semibold text-sm">Please select a pickup time & cab...</div>
             )}
           </div>
